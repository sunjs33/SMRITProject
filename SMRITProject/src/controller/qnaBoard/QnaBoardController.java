...
public class QnaBoardController extends HttpServlet implements Servlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (command.equals("/qna/qnaList.qna")) {
			...
			action.execute(request); //db에 있는값을 저장하기 위해
			RequestDispatcher dispatcher = request.getRequestDispatcher("/QNA_Board/qna_board_list.jsp");
			dispatcher.forward(request, response);
		}else if(command.equals("/qna/qnaWrite.qna")) {
			RequestDispatcher dispatcher=request.getRequestDispatcher("/QNA_Board/qna_board_write.jsp");
			dispatcher.forward(request, response);
			...
			action.execute(request);
			response.sendRedirect("qnaDetail.qna?num="+request.getParameter("boardNum"));
