...
public class SurveyOkAction {
	public void execute(HttpServletRequest request) {
		...
		res.setAge(Integer.parseInt(request.getParameter("res.age")));
		res.setLocation(request.getParameter("res.location"));
		data.setRes(res);
		
		/*
		 * data.getRes().setLocation(request.getParameter("res.location"));
		 * data.getRes().setAge(Integer.parseInt(request.getParameter("res.age")));
		 */
